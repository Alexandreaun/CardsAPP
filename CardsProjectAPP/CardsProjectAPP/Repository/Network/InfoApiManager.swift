...
protocol InfoRequestDelegate: class {
    func getApiInfoCards(completion: @escaping (InfoCardModel?, ValidationError?) -> Void)
    func getApiCardsDetail(_ typeCard: String, _ nameCard: String, completion: @escaping ([DetailCardModel]?, ValidationError?) -> Void)
}
...
class InfoApiManager: InfoRequestDelegate {
    ...
    func getApiCardsDetail(_ typeCard: String, _ nameCard: String, completion: @escaping ([DetailCardModel]?, ValidationError?) -> Void) {
        ...
        let urlString = "https://omgvamp-hearthstone-v1.p.rapidapi.com/cards/\(typeCard)/\(nameCard)?rapidapi-key=96a95fd7b6mshc4cf46eb9c7e4d7p1e2e40jsnb5be8b04e393"
        let error = ValidationError(titleError: "", messageError: "Não foi possível carregar os dados")
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            if httpResponse.statusCode == 200{
                ...
                do {
                    let decode = JSONDecoder()
                    decode.keyDecodingStrategy = .convertFromSnakeCase
                    let json = try decode.decode([DetailCardModel].self, from: datas)
                    completion(json, nil)
                }catch{
                    let error = ValidationError(titleError: "", messageError: "Não foi possível carregar os dados")
                    completion(nil, error)
